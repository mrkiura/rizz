# Rizz: Python Web Framework for building WSGI apps
...
def template_handler(req, resp):
    resp.body = app.template(
        "index.html", context={"name": "Rizz WSGI", "title": "Best Framework"}).encode()
```

### Unit Tests

The recommended way of writing unit tests is with [pytest](https://docs.pytest.org/en/latest/). There are two built in fixtures
that you may want to use when writing unit tests with Rizz WSGI. The first one is `app` which is an instance of the main `API` class:
...
You can create custom middleware classes by inheriting from the `rizz_wsgi.middleware.Middleware` class and overriding its two methods
